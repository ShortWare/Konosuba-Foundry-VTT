...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'konosuba', 'Konosuba', and 'KONOSUBA'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.konosuba/g),
        replacement: `game.${this.propName}`,
      },
      {
        pattern: new RegExp(/flags\.konosuba/g),
   ...
   * Rename files.
   *
   * Rename files that had konosuba in their name, such as
   * css/konosuba.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*konosuba*.*`).then((files) => {
      files.forEach((oldPath) => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(
          oldPath,
          `${directory}/${file.replaceAll("konosuba", this.packageName)}`,
          (err) => {
            if (err) throw err;
          }
        );
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-konosuba-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Konosuba's files so that we can process them.
    glob("*", { ignore: ["node_modules/**"] }).then((files) => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace konosuba name mentions in files.
      generator.replaceFileContents();
      // Rename files that had konosuba in their name.
